...
benchmarks = filter(d -> startswith(basename(d), r"benchmark_results-\d+-\d+"), readdir(data_dir; join=true))
...
energies = [JSON.parsefile(joinpath(data_dir, "energy-$(Int(freq))-$(Int(pow)).json"))["energy"] for (freq, pow) in zip(frequencies, powers)]
energy_fractions = energies ./ (powers ./ (3600 ./ times)) .* 100

times_plot = heatmap(
    frequencies_t, powers_t, reshape(times, length(powers_t), length(frequencies_t));
    title="Breeze.jl on $(gpu_name)",
